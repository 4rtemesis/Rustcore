...
-- Show in normal mode when durability pct is at or below this value
local LOW_THRESHOLD     = 0.20
...
local function BuildSlotFrame(parent, slotId)
    ...
    -- Sepia overlay (alpha driven by durability: 0 at 100%, 1 at LOW_THRESHOLD)
    ...
    f:SetScript("OnDragStop", function()
        ...
        if RustcoreDB then
            RustcoreDB.durHUDPos = { point = point, relPoint = relPoint, x = x, y = y }
        end
    end)
...
local function BuildHUD()
    ...
    if RustcoreDB and RustcoreDB.durHUDPos then
        local p = RustcoreDB.durHUDPos
    ...
    for _, entry in ipairs(slotEntries) do
        ...
        if link then
            ...
            if current ~= nil and maximum ~= nil and maximum > 0 then
                pct = current / maximum
                if showAll or pct <= LOW_THRESHOLD then
                    ...
                    -- Sepia alpha: 0 at 100% durability → 1 at LOW_THRESHOLD
                    local sepiaAlpha = 1.0 - Clamp(
                        (pct - LOW_THRESHOLD) / (1.0 - LOW_THRESHOLD), 0, 1)
                    entry.frame.sepiaTex:SetVertexColor(1.0, 0.5, 0.2, sepiaAlpha)

                    -- Rusted wipe: 0 coverage at LOW_THRESHOLD → full at 0%
                    local rustedFrac = Clamp(
                        (LOW_THRESHOLD - pct) / LOW_THRESHOLD, 0, 1)
...
evFrame:SetScript("OnEvent", function(_, event, unit)
    ...
    if hudContainer then UpdateHUD() end
end)
